...
$(document).ready(function(){
	...
	function initialize() {
		var mapOptions = {
			zoom: 8,
			center: new google.maps.LatLng(37.619592, -122.42189),
	...
	$(".menu ul li a").click(function(){
		...
		$("#content > div#"+current_page+"_page").fadeOut("fast", function(){
			...
			if (page != "home"){
				...
				$("#header").slideDown("medium", function(){
					...
					$(".menu").removeClass("menu_width_page");
				});
			}
			...
			if (page == 'about'){
				if ($("#content > div#"+page+"_page > .details.box").hasClass("animated")){
					$("#content > div#"+page+"_page > .details.box .item").removeClass("animated bounceIn");
				}
			}else if (page == 'resume'){
				if ($("#content > div#resume_page > .left_side").hasClass("animated")){
					$("#content > div#resume_page .left .item").removeClass("animated flipInX");
					$("#content > div#resume_page .awards .award").removeClass("animated flipInX");
					$("#content > div#resume_page .clients img").removeClass("animated flipInX");
				}
			}

			//fade in new page
			$("#content > div#"+page+"_page").fadeIn("fast", function(){
				...
				//portfolio page
				if (page == 'portfolio'){
					if (!$("#portfolio_grid").hasClass("animated")){
						$("#portfolio_grid").fadeIn("slow");
						$("#portfolio_grid").addClass("animated");
					}
				}
				...
				//blog page
				if (page != "blog"){
					$("#content > div#blog_page .posts .post").removeClass("animate");
				}
	...
	$("#sequence").hover(
		function(){
			$("#sequence .controls").fadeIn("fast");
		},
		function(){
			$("#sequence .controls").fadeOut("fast");
		}
	);
	...
	$(".view_portfolio_btn").click(function(){
		...
		$(".menu ul li a").removeClass("active");
		$('.menu ul li a[href="#portfolio"]').addClass("active");

		var page = $(".menu ul li a.active").attr("href");
		page = page.substring(1);
		...
		$("#content > div#"+current_page+"_page").fadeOut("fast", function(){
			...
			//fade in new page
			$("#content > div#"+page+"_page").fadeIn("fast");
			...
			//add active menu icon
			$('.menu ul li a[href="#portfolio"]').find("i").addClass("active");
	...
	$(".resume_slider_left, .resume_slider_right").click(function(){
	...
	$("body").on("click", "#portfolio_grid .img_outer", function(){
		...
		var description = $(this).prev().find(".description").html();
		var link = $(this).prev().find(".link").html();
	...
	$("body").on("click", ".close_portfolio_info", function(){
		$(this).parents("#portfolio_info").slideUp(function(){ $(this).remove(); });
	});
	...
	$("body").on("click touchstart", ".blog_view_btn", function(e){
		...
		$(".posts > .post").fadeOut("fast", function(){
			...
			$('.post[data-post="'+id+'"] .hidden').show();

			$('.post[data-post="'+id+'"]').removeClass("animate").addClass("fullOpacity");
			$('.post[data-post="'+id+'"]').fadeIn("fast");
			$('.post[data-post="'+id+'"]').addClass("active");


			$('.return_to_blog').css("display", "inline-block");
			$(".return_to_blog").attr("data-id", id);
	...
	$(".return_to_blog").click(function(){
		...
		$(".post.active").fadeOut("fast");
		$(".post.active").removeClass("active");
		$('.return_to_blog').fadeOut("fast");

		$(".posts > .post").fadeIn("fast").css("opacity", "0");

		var id = $(this).attr("data-id");

		$(".posts > .post .hidden").hide();


		$('.posts > .post[data-post="'+id+'"]').show();

		$(".pagination").fadeIn("fast");

		in_post_view = false;

		if ($(".posts").hasClass("animated")){
			$(".posts > .post").removeClass("animate");
			$(".posts > .post").addClass("fullOpacity");
		}
	...
	$(window).scroll(function(){
		...
		if($("#content > div#resume_page .clients").offset().top + 200 < $(window).scrollTop() + $(window).outerHeight() && $("#content > div#resume_page").is(":visible") && !$("#content > div#resume_page .clients").hasClass("animated")) {
		...
		if ($("#content > div#blog_page").is(":visible") && !$("#content > div#blog_page .posts").hasClass("animated") && !in_post_view){
			...
			$("#content > div#blog_page .posts > .post").each(function(i){
				...
				if($(this).offset().top + 300 < $(window).scrollTop() + $(window).outerHeight() && !$(this).hasClass("animated") && in_post_view == false) {
					...
					total_items++;
					if (total_items == $("#content > div#blog_page .posts > .post").length){
			...
			if (total_items == $("#content > div#blog_page .posts > .post").length && first_blog_view == false){
				$("#content > div#blog_page .posts").addClass("animated");
				$("#content > div#blog_page .posts > .post").removeClass("animated");
				$("#content > div#blog_page .posts > .post").removeClass("flipInX");
	...
	$(".tweets_box .pages i").click(function(){
	...
	$(".configs .btn").click(function(){

		if ($(this).parents(".configs").hasClass("active")){
			$(this).parents(".configs").animate({
				'left' : '-151px'
			});
			$(this).parents(".configs").removeClass("active");
		}else{
			$(this).parents(".configs").animate({
				'left' : '0'
			});
			$(this).parents(".configs").addClass("active");
		}

	});

	$(".configs .patterns img").click(function(){
	...
	$(".configs .colors span").click(function(){
